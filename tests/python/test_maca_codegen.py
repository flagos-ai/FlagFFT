...
def runtime():
    ...
    ), "filter a single physical device before running"
...
def test_leaf_matches_numpy(runtime, n, factors, lanes, dtype, direction, batch):
    ...
    for stage in range(1, len(factors)):
        ...
        angle = (
            (1 if direction == "inverse" else -1)
            * 2
            * np.pi
            * np.arange(factors[stage])[:, None]
            * (np.arange(count)[None, :] % prefix)
            / (prefix * factors[stage])
        )
        for table in (np.cos(angle), np.sin(angle)):
            args.append(
                torch.tensor(table.reshape(-1), dtype=real_dtype, device="cuda")
            )
    args.append(batch)
    scope[name][(math.ceil(batch / contiguous_batch_pack_for(plan)),)](
        *args, num_warps=2
    )
    expected = (
        np.fft.fft(x, axis=-1)
        if direction == "forward"
        else np.fft.ifft(x, axis=-1) * n
    )
