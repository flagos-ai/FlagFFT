...
TYPES = {"c2c": 0x29, "r2c": 0x2A, "c2r": 0x2C, "z2z": 0x69, "d2z": 0x6A, "z2d": 0x6C}
    ...
    try:
        ...
        for pattern in ("random", "impulse", "constant", "tone"):
            ...
            if pattern == "impulse":
                ...
                source = (
                    np.broadcast_to(np.cos(2 * np.pi * np.arange(n) / n), (batch, n))
                    .reshape(batch, *shape)
                    .copy()
                )
            source = source.astype(
                real_dtype if real_forward or real_inverse else complex_dtype
            )
            if real_inverse:
            ...
            directions = (
                (-1, 1)
                if not (real_forward or real_inverse)
                else (1 if real_inverse else -1,)
            )
            for direction in directions:
                if real_forward:
                    expected = np.fft.rfftn(source.astype(np.float64), axes=axes)
                elif real_inverse:
                    expected = n * np.fft.irfftn(
                        source.astype(np.complex128), s=shape, axes=axes
                    )
                elif direction == -1:
                ...
                output_bytes = (
                    ...
                    * (1 if real_inverse else 2)
                )
                storage = torch.full(
                    (output_bytes + 128,), 0xA5, dtype=torch.uint8, device="npu"
                )
                ...
                actual = (
                    ...
                    .numpy()
                    .copy()
                    .view(real_dtype if real_inverse else complex_dtype)
                )
                actual = actual.reshape(expected.shape)
                tolerance = 2e-11 if double else 3e-5
                scale = max(
                    1.0, float(np.linalg.norm(source.reshape(batch, -1), axis=1).max())
                )
                ...
                if not inplace:
                    ...
                    np.testing.assert_array_equal(
                        source_tensor.cpu().numpy(), raw.view(np.uint8)
                    )
    finally:
        torch.npu.synchronize()
