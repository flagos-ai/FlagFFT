...
def test_help(flagfft_cli) -> None:
    result = subprocess.run([str(flagfft_cli), "--help"], text=True, capture_output=True, check=False)
...
def test_rejects_integer_options_with_trailing_characters(invoke_cli, arguments) -> None:
...
def test_expressible_unsupported_case(invoke_cli, tmp_path) -> None:
    result, report = invoke_cli(
        "tune", "--api", "r2c", "--shape", "16", "--db", str(tmp_path / "tune.sqlite"),
        "--warmup", "0", "--iters", "1", "--static-limit", "1", "--finalists", "1",
    )
...
def test_unsupported_rank(invoke_cli) -> None:
    result, report = invoke_cli("bench", "--api", "c2c", "--shape", "16x16", "--iters", "1")
    assert result.returncode == 77
    assert report["status"] == "unsupported"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("--api", "d2z", "--direction", "forward", "--placement", "in-place", "--plan-api", "planmany"),
        ("--api", "c2r", "--direction", "inverse", "--placement", "in-place", "--plan-api", "planmany"),
        ("--api", "z2d", "--direction", "inverse", "--placement", "in-place"),
    ],
)
def test_correctness_supported_apis(invoke_cli, arguments) -> None:
    result, report = invoke_cli("test", "--suite", "correctness", "--shape", "16", "--batch", "2", *arguments)
...
def test_correctness_native_routes(invoke_cli) -> None:
    result, report = invoke_cli(
        "test", "--suite", "correctness", "--api", "c2c", "--direction", "forward",
        "--shape", "331", "--shape", "8192", "--batch", "1",
    )
    assert result.returncode == 0, report
...
@pytest.mark.parametrize("api,direction", [("c2c", "forward"), ("z2z", "inverse"), ("r2c", "forward"),
                                             ("d2z", "forward"), ("c2r", "inverse"), ("z2d", "inverse")])
def test_bench_smoke_apis(invoke_cli, api, direction) -> None:
    result, report = invoke_cli(
        "bench", "--api", api, "--direction", direction, "--shape", "16", "--batch", "1",
        "--warmup", "0", "--iters", "1", "--launches-per-sample", "1", "--print-path",
    )
    assert result.returncode == 0, report
    case = report["cases"][0]
...
def test_tune_retune_and_runtime_lookup(invoke_cli, tmp_path) -> None:
    ...
    tune_args = (
        "tune", "--api", "c2c", "--shape", "16", "--batch", "1",
        "--db", str(db), "--warmup", "0", "--iters", "1", "--static-limit", "1", "--finalists", "1",
    )
    ...
    result, report = invoke_cli(
        "bench", "--api", "c2c", "--shape", "16", "--batch", "1", "--warmup", "0",
        "--iters", "1", "--launches-per-sample", "1", "--print-path", env=env,
    )
    assert result.returncode == 0, report
    assert report["cases"][0]["correctness"]["passed"]
...
def test_tune_inverse_smoke(invoke_cli, tmp_path) -> None:
    result, report = invoke_cli(
        "tune", "--api", "c2c", "--direction", "inverse", "--shape", "16",
        "--db", str(tmp_path / "inverse.sqlite"), "--warmup", "0", "--iters", "1",
        "--static-limit", "1", "--finalists", "1",
    )
    assert result.returncode == 0, report
