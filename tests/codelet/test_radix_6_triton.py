"""
Unit tests for Triton radix-6 FFT implementation.
...
import time
import numpy as np
from src.codelet.triton.radix_6 import (
    triton_fwd_rad6_b1,
    triton_inv_rad6_b1,
    batch_fwd_rad6_b1,
    batch_inv_rad6_b1,
...
def torch_fft_rad6(x: torch.Tensor) -> torch.Tensor:
    """
    Reference 6-point FFT using torch.fft.

    Args:
        x: Input tensor of shape (..., 6)

    Returns:
        Output tensor with 6-point FFT applied.
    """
    if torch.is_complex(x):
        return torch.fft.fft(x, n=6, dim=-1)
    else:
        x_complex = torch.complex(x, torch.zeros_like(x))
        return torch.fft.fft(x_complex, n=6, dim=-1)


def torch_ifft_rad6(x: torch.Tensor) -> torch.Tensor:
    """
    Reference 6-point IFFT using torch.fft.

    Args:
        x: Input tensor of shape (..., 6)

    Returns:
        Output tensor with 6-point IFFT applied.
    """
    if torch.is_complex(x):
        return torch.fft.ifft(x, n=6, dim=-1)
    else:
        x_complex = torch.complex(x, torch.zeros_like(x))
        return torch.fft.ifft(x_complex, n=6, dim=-1)


class TestRadix6Correctness:
    """Correctness tests for radix-6 FFT."""
    ...
    def test_single_group_complex64(self):
        ...
        x = torch.tensor([1.0 + 2.0j, 3.0 + 4.0j, 5.0 + 6.0j,
                          7.0 + 8.0j, 9.0 + 10.0j, 11.0 + 12.0j], device=self.device)

        # Triton result
        result_triton = batch_fwd_rad6_b1(x)

        # torch.fft result
        result_torch = torch_fft_rad6(x)
    ...
    def test_batch_complex64(self):
        ...
        n_groups = 1024
        x = torch.randn(n_groups, 6, dtype=torch.complex64, device=self.device)
    ...
    def test_inverse_consistency_complex(self):
        ...
        Note: For radix-6, IFFT(FFT(x)) = x / 6
        """
        torch.manual_seed(42)
        n_groups = 1024
        x = torch.randn(n_groups, 6, dtype=torch.complex64, device=self.device)
        ...
        y = batch_fwd_rad6_b1(x)
        x_recovered = batch_inv_rad6_b1(y) / 6.0
    ...
    def test_specific_values_complex(self):
        ...
        test_cases = [
            # All ones -> [6, 0, 0, 0, 0, 0]
            (
                [1.0 + 0.0j, 1.0 + 0.0j, 1.0 + 0.0j, 1.0 + 0.0j, 1.0 + 0.0j, 1.0 + 0.0j],
                [6.0 + 0.0j, 0.0 + 0.0j, 0.0 + 0.0j, 0.0 + 0.0j, 0.0 + 0.0j, 0.0 + 0.0j],
            ),
