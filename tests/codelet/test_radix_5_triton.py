"""
Unit tests for Triton radix-5 FFT implementation.
...
import time
import numpy as np
from src.codelet.triton.radix_5 import (
    triton_fwd_rad5_b1,
    triton_inv_rad5_b1,
    batch_fwd_rad5_b1,
    batch_inv_rad5_b1,
...
def torch_fft_rad5(x: torch.Tensor) -> torch.Tensor:
    """
    Reference 5-point FFT using torch.fft.

    Args:
        x: Input tensor of shape (..., 5)

    Returns:
        Output tensor with 5-point FFT applied.
    """
    if torch.is_complex(x):
        return torch.fft.fft(x, n=5, dim=-1)
    else:
        x_complex = torch.complex(x, torch.zeros_like(x))
        return torch.fft.fft(x_complex, n=5, dim=-1)


def torch_ifft_rad5(x: torch.Tensor) -> torch.Tensor:
    """
    Reference 5-point IFFT using torch.fft.

    Args:
        x: Input tensor of shape (..., 5)

    Returns:
        Output tensor with 5-point IFFT applied.
    """
    if torch.is_complex(x):
        return torch.fft.ifft(x, n=5, dim=-1)
    else:
        x_complex = torch.complex(x, torch.zeros_like(x))
        return torch.fft.ifft(x_complex, n=5, dim=-1)


class TestRadix5Correctness:
    """Correctness tests for radix-5 FFT."""
    ...
    def test_single_group_complex64(self):
        ...
        x = torch.tensor([1.0 + 2.0j, 3.0 + 4.0j, 5.0 + 6.0j, 7.0 + 8.0j, 9.0 + 10.0j], device=self.device)

        # Triton result
        result_triton = batch_fwd_rad5_b1(x)

        # torch.fft result
        result_torch = torch_fft_rad5(x)
    ...
    def test_batch_complex64(self):
        ...
        n_groups = 1024
        x = torch.randn(n_groups, 5, dtype=torch.complex64, device=self.device)
    ...
    def test_inverse_consistency_complex(self):
        ...
        Note: For radix-5, IFFT(FFT(x)) = x / 5
        """
        torch.manual_seed(42)
        n_groups = 1024
        x = torch.randn(n_groups, 5, dtype=torch.complex64, device=self.device)
        ...
        y = batch_fwd_rad5_b1(x)
        x_recovered = batch_inv_rad5_b1(y) / 5.0
    ...
    def test_specific_values_complex(self):
        ...
        test_cases = [
            ...
            (
                ...
                [5.0 + 0.0j, 0.0 + 0.0j, 0.0 + 0.0j, 0.0 + 0.0j, 0.0 + 0.0j],
            ),
