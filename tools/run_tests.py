    ...
    for op in ops:
        ...
        for phase in phases:
            ...
            op_results[op["id"]][phase] = {
                "status": "NotFound" if enabled else "NotRun",
                "reference": (
...
def main(argv: list[str] | None = None) -> int:
    ...
    cases = expand_test_cases(
        ops, matrix, args.combination, args.scales, parse_shape_filter(args.shapes)
    )
    if args.max_cases is not None:
        cases = cases[: args.max_cases]
    active_ids = {case["op_id"] for case in cases}
    ops = [op for op in ops if op["id"] in active_ids]
    if not cases:
        raise ValueError("no test cases selected")
    perf_cases = performance_cases(cases)
    pinfo(
        f"Expanded {len(cases)} accuracy cases and {len(perf_cases)} performance cases from {len(ops)} operators"
    )
    if args.dry_run:
        print(
            json.dumps(
    ...
    started = time.monotonic()
    messages = []
    total = (0 if args.performance_only else len(cases)) + (
    ...
    with csv_path.open("w", newline="") as stream:
        ...
        try:
            ...
            if any(worker.is_alive() for worker in WORKER_PROCESSES):
                terminate_workers()
    results = aggregate_results(
        messages, ops, cases, not args.performance_only, not args.accuracy_only
    )
