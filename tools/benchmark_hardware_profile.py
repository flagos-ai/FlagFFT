...
def main():
    ...
    if (
        args.repeats < 1
        or args.timeout <= 0
        or any(p not in {"legacy", "native", "packed", "balanced"} for p in policies)
    ...
    if args.shapes:
        ...
        shapes = [
            ...
            if "x".join(map(str, shape)) in selected
        ]
        if not shapes:
            ...
            for api in args.apis.split(","):
                ...
                for repeat in range(args.repeats):
                    ...
                    for policy in order:
                        ...
                        env = dict(
                            os.environ,
                            FLAGFFT_EXECUTION_POLICY=policy,
                            FLAGFFT_TUNE_DISABLE="1",
                        )
                        active_build = baseline if policy == "legacy" else build
                        env["PYTHONPATH"] = (
                            str(active_build.parent / "python")
                            + os.pathsep
                            + env.get("PYTHONPATH", "")
                        )
                        record = dict(
                            case=case_id, policy=policy, repeat=repeat, status="failed"
                        )
                        try:
                            if policy not in correct:
                                ...
                                proc = subprocess.run(
                                    acceptance.build_accuracy_cmd(
                                        case,
                                        active_build / "ctest/numpy_fft_capture",
                                        directory,
                                        "flagfft",
                                    ),
                                    env=env,
                                ...
                                (directory / "accuracy.log").write_text(
                                    proc.stdout + proc.stderr
                                )
                                if proc.returncode:
                                    raise RuntimeError(
                                        f"capture exit {proc.returncode}"
                                    )
                                actual = acceptance.load_raw(
                                    directory / "flagfft.bin", api, shape, batch
                                )
                                stats = acceptance.judged_stats(
                                    acceptance.error_stats(
                                        actual, expected, expected.size // batch, batch
                                    ),
                                    acceptance.accuracy_limit(
                                        api, acceptance.product(shape)
                                    ),
                                )
                                (directory / "accuracy.json").write_text(
                            ...
                            if args.in_process_repeats:
                                ...
                                cmd[shape_index] = ",".join(
                                    [cmd[shape_index]] * args.repeats
                                )
                            ...
                            result = series[policy][
                                repeat if args.in_process_repeats else 0
                            ]
                            ...
                            record.update(
                                status="passed",
                                flagfft_ms=result["timing"]["flagfft_median_ms"],
                                platform_ms=result["timing"]["ref_median_ms"],
                                plan=result.get("plan_description", ""),
                            )
                        except (
                        ...
                        print(
                            json.dumps(
                                {k: v for k, v in record.items() if k != "plan"}
                            ),
                            flush=True,
                        )
    ...
    for case_id in sorted({r["case"] for r in records}):
        ...
        for policy in policies:
            trials = [
                r for r in records if r["case"] == case_id and r["policy"] == policy
            ]
            if len(trials) == args.repeats and all(
                r["status"] == "passed" for r in trials
            ):
        ...
        comparison[case_id] = {
            ...
            "speedup_vs_legacy": {
                ...
                if "legacy" in medians and p != "legacy"
            },
        }
    (root / "summary.json").write_text(
        json.dumps(
            {
                "cases": comparison,
                "failed_records": sum(r["status"] != "passed" for r in records),
            },
            indent=2,
        )
    )
