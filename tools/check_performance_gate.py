...
def audit(result_dir, threshold, expected_count=None):
    ...
    for case in expected:
        ...
        for label, records in (("accuracy", accuracy), ("platform_accuracy", platform)):
            if any(records.get(case_id, {}).get("status") != "Passed" for case_id in accuracy_ids):
        ...
        values = [perf.get(key) for key in ("flagfft_median_ms", "ref_median_ms", "speedup")]
        valid_numbers = all(isinstance(x, (int, float)) and math.isfinite(x) and x > 0 for x in values)
        ...
        rows.append({
            "case_id": case["case_id"], "flagfft_median_ms": values[0],
            "ref_median_ms": values[1], "speedup": values[2],
            "threshold": threshold, "passed": not reasons, "reason": "; ".join(reasons),
        })
    complete = expected_count is None or len(rows) == expected_count
    return {"passed": bool(rows) and complete and all(row["passed"] for row in rows),
            "expected_count": expected_count, "case_count": len(rows),
            "count_matches": complete, "passed_count": sum(row["passed"] for row in rows),
            "threshold": threshold, "cases": rows}
...
def main():
    ...
    report = audit(args.result_dir, args.threshold, args.expected_count)
    (args.result_dir / "performance_gate.json").write_text(json.dumps(report, indent=2) + "\n")
    with (args.result_dir / "performance_gate.csv").open("w", newline="") as stream:
        fields = ("case_id", "flagfft_median_ms", "ref_median_ms", "speedup", "threshold", "passed", "reason")
        writer = csv.DictWriter(stream, fieldnames=fields)
        writer.writeheader()
        writer.writerows(report["cases"])
    print(f"{'PASS' if report['passed'] else 'FAIL'}: {report['passed_count']}/{report['case_count']} cases at >= {args.threshold:g}")
