...
std::string LeafPlanNode::describe(int indent) const {
  std::ostringstream oss;
  oss << indent_str(indent) << "LeafPlan(n=" << length << ", factors=" << factors_str(factors)
...
std::string StockhamPlanNode::describe(int indent) const {
  std::ostringstream oss;
  oss << indent_str(indent) << "Stockham(n=" << length << ", factors=" << factors_str(factors) << ")";
  return oss.str();
}

std::string FourStepPlanNode::describe(int indent) const {
  std::ostringstream oss;
  oss << indent_str(indent) << "FourStep(n=" << length << ", n1=" << n1 << ", n2=" << n2 << ")\n";
...
std::string BluesteinPlanNode::describe(int indent) const {
  std::ostringstream oss;
  oss << indent_str(indent) << "Bluestein(n=" << length << ", conv_length=" << conv_length << ")\n";
...
PlanKey PlanKey::from_node(const PlanNodePtr &node) {
  ...
  throw std::runtime_error("unsupported plan node kind for key generation: " +
