...
PlanNodePtr plan_node_from_json(PlanBuilder &builder, const nlohmann::json &node) {
  ...
  if (kind == "ct_leaf") {
    ...
    int64_t lanes =
        node.contains("lanes") ? node["lanes"].get<int64_t>() : builder.choose_lanes(length, factors);
    int64_t num_warps =
        node.contains("num_warps") ? node["num_warps"].get<int64_t>() : builder.choose_num_warps(lanes);
    ...
    if (node.contains("generic_radices")) {
      ...
      for (int64_t radix : factors) {
        if (!contains(kSpecializedButterflyRadices, radix) &&
            !contains(kSpecializedDirectCodeletRadices, radix) && !contains(generic_radices, radix)) {
          generic_radices.push_back(radix);
        }
      }
      std::sort(generic_radices.begin(), generic_radices.end());
    }
    int64_t smem_size = node.contains("smem_size") ? node["smem_size"].get<int64_t>()
                                                   : (factors.size() <= 1 ? 0 : ceil_power_of_two(length));
    return std::make_shared<LeafPlanNode>(length,
                                          std::move(factors),
                                          remainder,
                                          lanes,
                                          num_warps,
  ...
  if (kind == "four_step") {
    ...
    return std::make_shared<FourStepPlanNode>(length,
                                              n1,
                                              n2,
  ...
  if (kind == "bluestein") {
    ...
    return std::make_shared<BluesteinPlanNode>(length,
                                               conv_length,
