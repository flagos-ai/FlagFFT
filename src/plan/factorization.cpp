...
std::vector<int64_t> PlanBuilder::select_leaf_factors(int64_t n) {
  ...
  const char *ix_short_factors = std::getenv("FLAGFFT_IX_SHORT_FACTORS");
  if (context.device_type == "ix" && context.batch == 1 &&
      context.input_dtype == "complex64" && ix_short_factors && std::string(ix_short_factors) == "1") {
    if (n == 64) return {8, 8};
    if (n == 128) return {16, 8};
    if (n == 1024) return {16, 8, 8};
  }
...
std::optional<int64_t> PlanBuilder::leaf_smem_elements(int64_t n,
  ...
  if (input_dtype != "complex64" && input_dtype != "float32" && input_dtype != "complex128" &&
      input_dtype != "float64") {
    return std::nullopt;
  }
  if (factors.size() <= 1) {
...
bool PlanBuilder::should_use_leaf(int64_t n, const std::vector<int64_t> &factors) {
  ...
  return n <= kLeafMaxN && smem_bytes.has_value() && *smem_bytes <= context.max_dynamic_smem_bytes;
}
...
PlanNodePtr PlanBuilder::make_leaf_plan(int64_t n, const std::vector<int64_t> &factors, int64_t rem) {
  ...
  if (!smem_elements.has_value()) {
    throw std::runtime_error(
        "ct_leaf shared-memory planner currently supports only float32/complex64 and float64/complex128 "
        "inputs");
  }
  int64_t lanes = choose_lanes(n, factors);
  ...
  for (int64_t radix : factors) {
    if (!contains(kSpecializedButterflyRadices, radix) &&
        !contains(kSpecializedDirectCodeletRadices, radix) &&
        std::find(generic_radices.begin(), generic_radices.end(), radix) == generic_radices.end()) {
