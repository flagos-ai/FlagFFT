...
std::string device_capabilities_json() {
  ...
  CUdevice device = 0;
  check(cuDeviceGet(&device, index), "device capabilities");
  auto attribute = [&]( CUdevice_attribute key) -> nlohmann::json {
    int value = 0;
    if (cuDeviceGetAttribute(&value, key, device) != CUDA_SUCCESS) return nullptr;
    return value;
  };
  char name[256] = {};
  ...
  nlohmann::json result = {
      ...
      {"warp_size", attribute(CU_DEVICE_ATTRIBUTE_WARP_SIZE)},
      {"max_threads_per_block", attribute(CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_BLOCK)},
      {"shared_memory_per_block", attribute(CU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK)},
      {"max_dynamic_shared_memory", attribute(CU_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK_OPTIN)},
