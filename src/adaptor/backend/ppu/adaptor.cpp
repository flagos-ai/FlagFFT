...
std::string device_capabilities_json() {
  ...
  HGdevice device = 0;
  check(hgDeviceGet(&device, index), "device capabilities");
  auto attribute = [&]( HGdevice_attribute key) -> nlohmann::json {
    int value = 0;
    if (hgDeviceGetAttribute(&value, key, device) != HGGC_SUCCESS) return nullptr;
    return value;
  };
  char name[256] = {};
  check(hgDeviceGetName(name, sizeof(name), device), "device name");
  nlohmann::json result = {
      {"schema_version", 1}, {"source", "driver_query"},
      {"backend", backend_name()}, {"device_index", index},
      {"device_name", name}, {"device_arch", arch},
      {"warp_size", attribute(HG_DEVICE_ATTRIBUTE_WARP_SIZE)},
      {"max_threads_per_block", attribute(HG_DEVICE_ATTRIBUTE_MAX_THREADS_PER_BLOCK)},
      {"shared_memory_per_block", attribute(HG_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK)},
      {"max_dynamic_shared_memory", attribute(HG_DEVICE_ATTRIBUTE_MAX_SHARED_MEMORY_PER_BLOCK_OPTIN)},
      {"multiprocessor_count", attribute(HG_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT)}
  };
  return result.dump();
}
