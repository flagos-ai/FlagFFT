...
void print_usage() {
  std::cout << "Usage: flagfft-cli test --suite plan|api-errors|correctness|all [case options]\n"
               "       flagfft-cli bench [case options] --warmup W --iters K --launches-per-sample K "
               "[--print-path]\n"
               "       flagfft-cli tune [case options] --db PATH --warmup W --iters K --static-limit N "
               "--finalists N [--retune]\n"
...
Args parse_args(int argc, char **argv) {
  ...
  for (int i = 2; i < argc; ++i) {
    ...
    if (argument == "--help" || argument == "-h") {
      ...
      args.timing.launches_per_sample =
          parse_positive("--launches-per-sample", need("--launches-per-sample"));
...
json run_plan_suite() {
  ...
  auto nested =
      std::dynamic_pointer_cast<flagfft::FourStepPlanNode>(builder.build(1 << 23, request_for(1 << 23)));
...
json run_api_errors_suite() {
  ...
  expect_flagfft(flagfftPlan1d(nullptr, 16, FLAGFFT_C2C, 1), FLAGFFT_INVALID_VALUE, "null plan output");
  expect_flagfft(flagfftPlan2d(&plan, 8, 8, FLAGFFT_C2C), FLAGFFT_NOT_SUPPORTED, "unsupported plan2d");
  expect_flagfft(flagfftPlan3d(&plan, 4, 4, 4, FLAGFFT_C2C), FLAGFFT_NOT_SUPPORTED, "unsupported plan3d");
  int dims[2] = {8, 8};
  expect_flagfft(flagfftPlanMany(&plan, 2, dims, nullptr, 1, 64, nullptr, 1, 64, FLAGFFT_C2C, 1),
                 FLAGFFT_NOT_SUPPORTED,
                 "unsupported rank-2 planmany");
  check_flagfft(flagfftPlan1d(&plan, 16, FLAGFFT_C2C, 1), "create validation plan");
  expect_flagfft(flagfftExecC2C(plan, nullptr, nullptr, FLAGFFT_FORWARD),
                 FLAGFFT_INVALID_VALUE,
                 "null exec buffers");
  expect_flagfft(flagfftExecC2C(plan,
                                reinterpret_cast<flagfftComplex *>(0x1),
                                reinterpret_cast<flagfftComplex *>(0x2),
                                0),
                 FLAGFFT_INVALID_VALUE,
                 "invalid direction");
  expect_flagfft(flagfftExecZ2Z(plan,
  ...
  int one[1] = {16};
  int padded[1] = {18};
  check_flagfft(flagfftPlanMany(&plan, 1, one, padded, 1, 18, nullptr, 1, 9, FLAGFFT_R2C, 1),
                "supported padded r2c planmany");
...
int execute(const Args &args) {
  ...
  if (args.command == "test" && args.suite == "all") {
    ...
    CaseSpec z2z = c2c;
    z2z.api = FftApi::Z2Z;
    z2z.direction = FLAGFFT_INVERSE;
    CaseSpec r2c = c2c;
    r2c.api = FftApi::R2C;
    CaseSpec d2z = c2c;
    d2z.api = FftApi::D2Z;
    d2z.placement = Placement::InPlace;
    d2z.plan_api = PlanApi::PlanMany;
    CaseSpec c2r = c2c;
    c2r.api = FftApi::C2R;
    c2r.direction = FLAGFFT_INVERSE;
    c2r.placement = Placement::InPlace;
    c2r.plan_api = PlanApi::PlanMany;
    CaseSpec z2d = c2c;
    z2d.api = FftApi::Z2D;
    z2d.direction = FLAGFFT_INVERSE;
  ...
  if (run_cases) {
    const Operation operation = args.command == "bench"  ? Operation::Bench
                                : args.command == "tune" ? Operation::Tune
                                                         : Operation::Test;
  ...
  if (args.command == "test") {
    ...
    report["cases"] = run_tune_cases(cases,
                                     args.db,
                                     args.timing.warmup,
                                     args.timing.iters,
                                     args.static_limit,
