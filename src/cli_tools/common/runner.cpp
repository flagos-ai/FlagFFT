...
namespace {
  ...
  FlagfftPlanHandle make_flagfft_plan(const CaseSpec &spec, const BufferLayout &layout) {
    ...
    if (spec.plan_api == PlanApi::Plan1d) {
      ...
      result = is_real_forward_api(spec.api) ? flagfftPlanMany(&raw,
                                                               1,
                                                               dims,
                                                               padded,
                                                               1,
  ...
  CufftPlanHandle make_cufft_plan(const CaseSpec &spec, const BufferLayout &layout) {
    ...
    if (spec.plan_api == PlanApi::PlanMany) {
      ...
      int padded[1] = {layout.padded};
      int compact[1] = {layout.half};
      check_cufft(is_real_forward_api(spec.api) ? cufftPlanMany(plan.put(),
                                                                1,
                                                                dims,
                                                                padded,
                                                                1,
      ...
      check_cufft(cufftPlan1d(plan.put(), layout.n, cufft_type(spec.api), spec.batch), "create cuFFT plan1d");
    }
    return plan;
  }
  ...
  void exec_cufft(cufftHandle plan, const CaseSpec &spec, void *input, void *output) {
    ...
    switch (spec.api) {
      ...
      case FftApi::Z2Z:
        ...
        break;
      case FftApi::R2C:
        check_cufft(cufftExecR2C(plan, static_cast<cufftReal *>(input), static_cast<cufftComplex *>(output)),
                    "cufftExecR2C");
        break;
      case FftApi::D2Z:
        check_cufft(cufftExecD2Z(plan,
                                 static_cast<cufftDoubleReal *>(input),
                                 static_cast<cufftDoubleComplex *>(output)),
                    "cufftExecD2Z");
        break;
      case FftApi::C2R:
        check_cufft(cufftExecC2R(plan, static_cast<cufftComplex *>(input), static_cast<cufftReal *>(output)),
                    "cufftExecC2R");
        break;
      case FftApi::Z2D:
        check_cufft(cufftExecZ2D(plan,
                                 static_cast<cufftDoubleComplex *>(input),
  ...
  struct Execution {
    explicit Execution(const CaseSpec &s, bool explicit_stream)
        : spec(s), layout(layout_for(s)), ff_in(layout.allocation_bytes), cf_in(layout.allocation_bytes) {
    ...
    void *cf_output() {
      return spec.placement == Placement::InPlace ? cf_in.get() : cf_out.get();
    }
  ...
  json compare_outputs(Execution &run) {
    const std::size_t bytes =
        run.spec.placement == Placement::InPlace ? run.layout.allocation_bytes : run.layout.out_bytes;
    double max_abs = 0.0;
    double sum_sq = 0.0;
      ...
      for (int b = 0; b < run.spec.batch; ++b) {
        const int count = is_real_inverse_api(run.spec.api)   ? run.layout.n
                          : is_real_forward_api(run.spec.api) ? 2 * run.layout.half
                                                              : 2 * run.layout.n;
        const int stride = run.spec.placement == Placement::InPlace && is_real_inverse_api(run.spec.api)
                               ? run.layout.padded
                               : count;
...
json run_correctness(const CaseSpec &spec, bool include_path) {
  ...
  json result = {
      {       "case",      case_json(spec)},
      {"correctness", compare_outputs(run)}
  };
  if (include_path) {
...
json run_benchmark(const CaseSpec &spec, const TimingConfig &timing, bool include_path) {
  ...
  json result = {
      ...
      {     "timing",
       ...
       {"flagfft_median_ms", ff_median},
       {"flagfft_p90_ms", percentile(flagfft_times, 0.9)},
       {"cufft_median_ms", cf_median},
       {"cufft_p90_ms", percentile(cufft_times, 0.9)},
