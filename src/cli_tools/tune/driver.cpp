...
namespace {
  ...
  json tune_one(const CaseSpec &spec,
    ...
    if (supported.empty()) {
      throw CliException("no supported tune candidates for length " + std::to_string(n), kExitRuntimeError);
    }
    ...
    for (auto &candidate : supported) {
      ...
      try {
        scores.push_back(
            {candidate,
             bench_candidate(n, spec.batch, direction, warmup, iters, plan_json, device_index, device_arch),
             {},
             plan_json,
             false});
    ...
    if (scores.empty()) {
      throw CliException("all tune candidates failed for length " + std::to_string(n), kExitRuntimeError);
    }
    ...
    for (int i = 0; i < std::min(finalists, static_cast<int>(scores.size())); ++i) {
      scores[i].error =
          verify_against_cufft(n, spec.batch, direction, scores[i].plan_json, device_index, device_arch);
...
json run_tune_cases(const std::vector<CaseSpec> &cases,
  ...
  for (const auto &spec : cases) {
    outputs.push_back(
        tune_one(spec, db_path, warmup, iters, static_limit, finalists, retune, device_index, device_arch));
  }
  return outputs;
}
