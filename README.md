...
for those stages. Inverse leaves reuse the same codelets through the standard
`conj(FFT(conj(x)))` identity while direction-specific twiddle and DFT tables
are generated by the C++ codegen layer.
...
The implemented compute paths are currently 1-D `flagfft.fft` and
`flagfft.ifft` for CUDA tensors on the last dimension. The remaining
torch.fft-compatible entrypoints are present as API stubs and raise
`NotImplementedError` until their C++ plan/exec paths are added.
...
offline tuning, and `fft_with_plan()` / `ifft_with_plan()` for benchmarking an
explicit plan.
...
flagfft-tune --api ifft --lengths 4096 --batch 256 --retune
```
...
flagfft.tune(flagfft.ifft, lengths=[4096], batch=256, retune=True)
```

The tuner writes `.flagfft/tuned_plans.sqlite` unless `--db` or the Python
`db=` argument is provided. Normal `flagfft.fft` and `flagfft.ifft` calls read
that default database automatically on cold plan-cache misses.
`FLAGFFT_TUNE_DB` overrides the path, and `FLAGFFT_TUNE_DISABLE=1` disables
tuned-plan lookup. Use
...
Only `fft` and `ifft` are currently benchmarkable; the other
torch.fft-compatible names are reserved and raise `NotImplementedError` in the
tune layer.
...
python benchmark/benchmark_fft_mixed_radix.py --api ifft --lengths 16 --batch 2 --warmup 1 --iters 3
```
