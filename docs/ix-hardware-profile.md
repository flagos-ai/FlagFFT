...
regressed about 10% and remains the documented follow-up. 1D/2D regression
passed 24/24 cases and pytest reported 166 passed / 140 skipped.
Evidence: `results/20260917_165501_maca_hw_profile/` (`REPORT.md`, `acc_tile/`,
`acc_v1/`, `perf_ab/`) and `results/20260917_170938_musa_hw_profile/`.
CUDA keeps `v2` and is intentionally not in the portable-tile set: on an
A100-SXM4-40GB (CUDA 13.2, Triton 3.6) `feature/hw-cuda` `a862264` produced
bitwise-identical outputs against `main` `7344b5f` for all 32 selected captures
(1D 8192, 2D 64x64/128x128, 3D 16^3/32^3/16x997x64/64^3, small Z2Z), so the
default `legacy` policy is unchanged there. 19 operators / 48 accuracy cases
passed, pytest reported 166 passed / 140 skipped, and candidate/baseline timing
ratios stayed within 0.99-1.01 outside timer noise with unchanged
cuFFT-relative speedups. Evidence: `results/20260917_165334_cuda_hw_profile/`
(`REPORT.md`, `summary.json`, `ab_bitwise/`).
