...
uintptr_t RefHandle::get() const {
  return impl_;
}

uintptr_t* RefHandle::ptr() {
  return &impl_;
}
...
void ref_plan_1d(RefHandle& plan, int nx, flagfftType type, int batch) {
  ...
  if (r != CUFFT_SUCCESS) {
    FAIL() << "cufftPlan1d(nx=" << nx << ", type=" << type << ", batch=" << batch << ") failed with code "
           << r;
  }
}
...
void ref_plan_2d(RefHandle& plan, int nx, int ny, flagfftType type) {
  ...
  if (r != CUFFT_SUCCESS) {
    FAIL() << "cufftPlan2d(nx=" << nx << ", ny=" << ny << ", type=" << type << ") failed with code " << r;
  }
}
...
void ref_plan_3d(RefHandle& plan, int nx, int ny, int nz, flagfftType type) {
  ...
  if (r != CUFFT_SUCCESS) {
    FAIL() << "cufftPlan3d(nx=" << nx << ", ny=" << ny << ", nz=" << nz << ", type=" << type
           << ") failed with code " << r;
  }
}
